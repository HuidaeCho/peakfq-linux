...
C     check table data set info and get table info
      CALL WDTBFN (WDMSFL,DSN,TABIND,TABNAM,
     O             TBCNT,LREC,TGRPPT,IDUM,RETCOD)
      write (*,*) 'WTBGET, WDTBFN: RETCOD ',RETCOD
C
      IF (RETCOD.EQ.0) THEN
C       get label
        LIND= WDRCGO(WDMSFL,LREC)
...
C       where does the table start
        ...
        CALL WDPTSP (TABPTR,
     O               DREC,DPOS)
       write (*,*) 'WTBGET, WDPTSP: DREC, DPOS',DREC,DPOS
        CALL WTBDSP (TABDIM,
     O               IDUM,TROW,TSPA,ASPA)
       write (*,*) 'WTBGET, WTBDSP: IDUM,TROW,TSPA,ASPA',
     $                              IDUM,TROW,TSPA,ASPA
        IF (FROW.GT.0 .AND. FSPA.GT.0) THEN
...
C         problems with row/space specs
      ...
      END IF
C
      write (*,*) 'WTBGET, check data header: RETCOD ',RETCOD
      IF (RETCOD.EQ.0) THEN
C       check data header
        DIND= WDRCGO(WDMSFL,DREC)
      write (*,*) 'WTBGET, WDRCGO: DREC, DIND ',DREC,DIND
        IF (TABID.NE.WIBUFF(DPOS,DIND) .OR.
     1      TABDIM.NE.WIBUFF(DPOS+1,DIND)) THEN
C         big problem, data header doesnt match
          RETCOD= -32
...
C
          ...
          GROW= 1
          GPOS= 0
        write (*,*) 'WTBGET, 10 LOOP '
 10       CONTINUE
C           loop to get data values
            RETCOD= 0
        write (*,*) 'WTBGET, in 10 LOOP, DPOS ',DPOS
            IF (DPOS.GT.512) THEN
C             get next record number
              DREC= WIBUFF(4,DIND)
C             bring next record into buffer
              DIND= WDRCGO(WDMSFL,DREC)
        write (*,*) 'WTBGET, in 10 LOOP, DIND ',DIND
              DPOS= DPOS- 508
            END IF
C
            CPOS= CPOS+ 1
        write (*,*) 'WTBGET, in 10 LOOP, CPOS, XSPA ',CPOS,XSPA
            IF (CPOS.GT.XSPA) THEN
C             row complete
              CROW= CROW+ 1
              IF (CROW.GT.LROW) THEN
...
C               begin next row
              ...
              END IF
            END IF
         write (*,*) 'WTBGET, in 10 LOOP: CROW,FROW,CPOS,FSPA ',
     $                                    CROW,FROW,CPOS,FSPA
            IF (CROW.LT.FROW .OR. CPOS.LT.FSPA .OR. CPOS.GT.LSPA) THEN
C             skip this one
              IF (RETCOD.NE.1) RETCOD= -1
...
C             get ready to read the next value
              ...
              IF (GPOS.GT.NSPA) THEN
                ...
                GROW= GROW+ 1
              END IF
         write (*,*) 'WTBGET, in 10 LOOP: GPOS,GROW,DPOS,DIND ',
     $                                    GPOS,GROW,DPOS,DIND
         write (*,*) 'WTBGET, in 10 LOOP: WRBUFF ',WRBUFF(DPOS,DIND)
         write (*,*) 'WTBGET, in 10 LOOP: WIBUFF ',WIBUFF(DPOS,DIND)
              TBRBUF(GPOS,GROW)= WRBUFF(DPOS,DIND)
         write (*,*) 'WTBGET, in 10 LOOP: TBRBUF ',TBRBUF(GPOS,GROW)
            END IF
            DPOS= DPOS+ 1
          IF (RETCOD.LE.0) GO TO 10
C         reset return code for the outsize world
