...
167   FORMAT(54H ***WCF167E-HIST PERIOD NO LONGER THAN SYS + HIST PKS.,
  ...
  120 T=AMIN1(300., 5.*FLOAT(NSYS))
      ...
      IER=MAX0(IER,1)
      IF(MSL.GE.2 .AND. EMAOPT.EQ.0)WRITE(MSG,169)HISTPN,T
...
191   FORMAT(  '   *WCF191I-USER LOW-OUTLIER CRITERION SUPERSEDES ',
     $         ' LIMIT CALCULATED BY ',/,
     $         '            AUTOMATED LOW-OUTLIER DETECTION PROCEDURE.',
     $       4X,F8.1,1X,F8.1)
   10 CONTINUE
C
...
Cprh: expanded index array to include 1.5 yr (.6667) and 2.33 yr (.4292) floods
      DATA NINDX/19/, INDXPT/16,21,26,4,5,9,10,11,13,17,20,23,25,27,28,
     *                       29,30,31,32, 13*0 /
...
Cprh: added 1.5 yr (.6667) flood to probability array
      DATA  TXPROB /
     $ 0.9995, 0.9990, 0.9980, 0.9950, 0.9900, 0.9800, 0.9750, 0.9600,
     $ 0.9500, 0.9000, 0.8000, 0.7000, 0.6667, 0.6000, 0.5704, 0.5000,
     $ 0.4292, 0.4000, 0.3000, 0.2000, 0.1000, 0.0500, 0.0400, 0.0250,
     $ 0.0200, 0.0100, 0.0050, 0.0020, 0.0010, 0.0001, 0.00001,0.000001/
C
...
C         report Multiple GB LO messges
          IF (NLOW .GT. 0) THEN
            ...
            IF (nzero .GT. 0) WRITE(MSG,2005) nzero
            DO 10 I = 1,nlow
