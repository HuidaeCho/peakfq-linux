...
C       open file as 'read only'
        !WRITE(*,*) 'WDOPPC:WDBOPN:RWFLG:',RONWFG
        !WRITE(*,*) '   ',WDNAME
        OPEN (UNIT=WDMSFL,FILE=WDNAME,STATUS='OLD',
     1        ACTION='READ', SHARE='DENYNONE',
     2        ACCESS='DIRECT',FORM='UNFORMATTED',RECL=2048,
     3        ERR=10,IOSTAT=IOS)
...
C       first time called, initialize WDM record buffer
        !write(*,*) 'ADWDM:WDOPPC:WDBOPN:initialize'
        CALL WDBFIN
        INITFG= 1
      END IF
      IF (RONWFG.EQ.2) THEN
...
C       check WDM directory records
        CALL WDFLCK (WDMSFL,
     O               RETCOD)
        !write(*,*) 'ADWDM:WDOPPC:WDBOPN:check dir',retcod
      END IF
      GO TO 20
 10   CONTINUE
